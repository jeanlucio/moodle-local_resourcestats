...
Three areas currently have **zero** coverage, found while auditing the suite's real
attribution rather than trusting the headline percentage at face value:

- **`preferences\controller`** — the display-preferences form's controller
  (`handle_post()`, `get_template_context()`, `get_page_url()`) has no dedicated test file.
- **`lib.php`** — both navigation-extension callbacks
  (`local_resourcestats_extend_navigation_course()`,
  `local_resourcestats_extend_settings_navigation()`) are exercised only by manually
  clicking through the UI, never by PHPUnit.
- **`db/upgrade.php`** — the plugin's one real upgrade step (purging orphaned rows on
  install) is covered indirectly via `observer_test.php`'s call to the same underlying
  `observer::purge_orphaned_rows()` method, but `xmldb_local_resourcestats_upgrade()`
  itself is never invoked directly by a test.

None of these are exercised by Behat either — the suite is PHPUnit-only today. Closing
this gap is tracked as follow-up work, not silently left out of this page.
