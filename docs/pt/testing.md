...
| `local/completion_stats_test.php` | 15 | O helper de conclusão: quais estados contam como concluído em cada forma de configurar a atividade — conclusão manual conta só "concluído", conclusão automática que exige nota de aprovação exclui o estado reprovado, conclusão automática que aceita qualquer nota o inclui, e atividade com regra de conclusão própria o exclui de novo — cada uma espelhando a regra do próprio core, com um caso provando que o agregado concorda estudante a estudante com `cm_completion_details::is_overall_complete()`; o denominador conta os estudantes que o curso rastreia para conclusão e não o professor; atividade sem rastreamento fica ausente do resultado em vez de aparecer como zero; conclusão desligada no curso esconde os números mesmo com as atividades mantendo sua própria configuração; professor restrito a grupo tem contagem e denominador apenas do próprio grupo; a leitura por usuário, o conjunto de rastreados (curso inteiro e por grupo) e o rótulo por estudante para cada situação, incluindo um estudante que o curso não rastreia |
...
|----------------------|----------:|------------------|
| `local_resourcestats_teacher.feature` | 5 | Um professor ativa os badges de exibição pelo formulário real de preferências e os vê renderizados ao lado da atividade certa na página do curso; a visão geral de estatísticas do curso mostra o nome da atividade, o total de estudantes inscritos e o percentual de engajamento; o badge de conclusão conta apenas quem concluiu e aparece somente na atividade que rastreia conclusão; a página de estatísticas do curso reporta o mesmo número de conclusão por atividade; o badge de último estudante, único de largura variável, é renderizado depois dos numéricos |
