...
|---------|:--------------------:|
| `lib.php` | 0% (0/31) |
| `db/upgrade.php` | 0% (0/4) |

### Lacunas Conhecidas

Três áreas atualmente têm **zero** cobertura, encontradas ao auditar a atribuição real da
suíte em vez de confiar no percentual de cabeçalho à primeira vista:

- **`preferences\controller`** — o controller do formulário de preferências de exibição
  (`handle_post()`, `get_template_context()`, `get_page_url()`) não tem arquivo de teste
  dedicado.
- **`lib.php`** — os dois callbacks de extensão de navegação
  (`local_resourcestats_extend_navigation_course()`,
  `local_resourcestats_extend_settings_navigation()`) só são exercitados clicando
  manualmente na interface, nunca pelo PHPUnit.
- **`db/upgrade.php`** — o único passo real de upgrade do plugin (limpar linhas órfãs na
  instalação) é coberto indiretamente pela chamada de `observer_test.php` ao mesmo método
  subjacente `observer::purge_orphaned_rows()`, mas `xmldb_local_resourcestats_upgrade()`
  em si nunca é invocado diretamente por um teste.

Nenhuma dessas áreas é exercitada por Behat tampouco — a suíte é somente PHPUnit hoje.
Fechar essa lacuna está registrado como trabalho futuro, não omitido silenciosamente
desta página.
